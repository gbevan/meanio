...
su - mean -c "cd appserver && gulp cssmin && gulp uglify"

echo `date` "Starting MEAN.IO Appserver. NODEARGS:${NODEARGS} MEANARGS:${MEANARGS}"
su - mean -c "cd appserver && NODE_ENV=production forever --spinSleepTime=5000 --killSignal=SIGTERM ${FOREVERARGS} -c 'node ${NODEARGS}' server.js ${MEANARGS}"
